...
def scrape_archetype_maindeck(archetype_url: str) -> list[dict]:
    """Extract maindeck cards from MTGGoldfish archetype page.
    
    Extracts cards from all maindeck sections (Creatures, Planeswalkers, Spells, 
    ...
    if not soup:
        _log.warning(f"Failed to fetch maindeck data from {archetype_url}")
        return []
    
    maindeck_cards = []
    maindeck_categories = ["Creatures", "Planeswalkers", "Spells", "Artifacts", "Lands", "Enchantments"]
    
    # Find all h3 headings
    all_h3 = soup.find_all("h3")
    
    # Process each maindeck category
    for h3 in all_h3:
        h3_text = h3.get_text(strip=True)
        
        # Skip sideboard sections and non-category headings
        if "Sideboard" in h3_text:
            continue
        if h3_text not in maindeck_categories:
            continue
        
        # Find the parent container
        container = h3.find_parent("div", class_="spoiler-card-container")
        if not container:
            continue
        
        # Find all card divs that are siblings of this h3
        card_divs = []
        ...
        while current:
            if hasattr(current, 'name'):
                if current.name == "h3":
                    ...
                    break
                if current.get("class") and "spoiler-card" in current.get("class", []):
                    card_divs.append(current)
    ...
    _log.debug(f"Maindeck: Found {len(maindeck_cards)} cards")
    return maindeck_cards
...
def scrape_meta(fmt="standard", limit: int | None = None, throttle: bool = False) -> list[MinimalDeck]:
    ...
    for idx, ((deck, archetype_url), meta) in enumerate(zip(decks, metas)):
        ...
        _log.debug(f"Processing deck {idx + 1}/{len(decks)}: {deck.name}")
        
        maindeck_data = scrape_archetype_maindeck(archetype_url)
        if maindeck_data:
            deck.update_metadata(maindeck=maindeck_data)
        _log.debug(f"  Maindeck: {len(maindeck_data)} cards")
        
        sideboard_data = scrape_archetype_sideboard(archetype_url)
        if sideboard_data:
            deck.update_metadata(sideboard=sideboard_data)
        _log.debug(f"  Sideboard: {len(sideboard_data)} cards")
